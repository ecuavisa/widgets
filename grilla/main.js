...
$('#martes-data').sheetrock({
    ...
    callback: function (data, sheetrock, response) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            //console.log("Hora actual:" + horaActual);
            if (hora == horaActual && horaActual <= horaSiguiente && nombre == nombreSiguiente) {
                console.log("si paso martes");
                $('#martes-data tr').eq(rowIndex).addClass("bg-success");
            }
            rowIndex++;
        }
...
$('#miercoles-data').sheetrock({
    ...
    callback: function (data, sheetrock, response) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            //console.log("Hora actual:" + horaActual);
            if (hora == horaActual && horaActual <= horaSiguiente && nombre == nombreSiguiente) {
                console.log("si paso miercoles");
                $('#miercoles-data tr').eq(rowIndex).addClass("bg-success");
            }
            rowIndex++;
        }
